...
export const Stats: React.FC<StatsProps> = () => {
  ...
  const sortedDayData = useMemo(() => {
    const mappedData = daysInMonth.map(day => {
      ...
      const activeUsersOnDay = allSchedules.filter(userSched => {
        ...
        return daySched && daySched.ranges.length > 0;
      });
    ...
    const data = mappedData.filter((item): item is NonNullable<typeof item> => item !== null);

    // Sort: Max Overlap Descending -> Date Ascending
  ...
  const DayRow: React.FC<{ data: typeof sortedDayData[0] }> = ({ data }) => {
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
        <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
          <div className="flex-shrink-0 w-24 bg-brand-50 rounded-2xl p-2 text-center border border-brand-100">
             <div className="text-xl font-extrabold text-brand-500">{format(day, 'M/d')}</div>
             <div className="text-xs font-bold text-brand-300 uppercase">{format(day, 'EEEE', { locale: zhTW })}</div>
          </div>
          
          <div className="flex-1">
             <div className="flex flex-wrap gap-2">
                {activeUsersOnDay.map(us => {
                  const u = users.find(u => u.name === us.userName);
                  return (
                    <span key={us.userName} className="inline-flex items-center gap-1.5 bg-slate-50 px-3 py-1 rounded-full text-sm font-bold border border-slate-200 text-slate-600">
                      <span>{u?.emoji || '👤'}</span>
                      <span className="truncate max-w-[80px]">{u?.name || us.userName}</span>
                    </span>
                  );
                })}
             </div>
           ...
           <div className="relative h-auto space-y-2">
             ...
             <div className="absolute inset-0 flex pointer-events-none z-0">
               {Array.from({ length: 4 }).map((_, i) => (
                 <div key={i} className="flex-1 border-l border-slate-100 last:border-r"></div>
               ))}
             </div>

             {/* Overlap Highlights (Vertical Bars - Heatmap) */}
             <div className="absolute inset-0 flex z-0 h-full pointer-events-none rounded-xl overflow-hidden">
                {overlapHeatmap.map((h, idx) => (
                  <div 
                    key={idx} 
                    className="flex-1 transition-all"
                    style={{ 
                      // Using Slate-600 (Grey Blue) for heatmap
                      // RGB(71, 85, 105)
                      backgroundColor: h.count > 1 ? `rgba(71, 85, 105, ${h.count / activeUsersOnDay.length * 0.5})` : 'transparent' 
                    }}
                  ></div>
                ))}
             </div>

             {/* Individual User Bars */}
             {activeUsersOnDay.map((uSched) => {
               const daySched = uSched.schedules.find(d => d.date === data.dateStr);
               const u = users.find(user => user.name === uSched.userName);
               if (!daySched) return null;

               return (
                 <div key={uSched.userName} className="relative h-8 w-full bg-slate-50/50 rounded-xl flex items-center overflow-hidden border border-slate-100/50">
                    ...
                    {/* Render Time Blocks */}
                    {daySched.ranges.map((range, rIdx) => {
             ...
             {/* Intersection Result Bar (Summary) */}
             {activeUsersOnDay.length > 1 && maxOverlap > 1 && (
               <div className="relative h-6 w-full mt-3 rounded-xl overflow-hidden flex items-center bg-slate-100 border border-slate-200">
                 <div className="absolute left-0 px-3 text-[10px] text-slate-500 font-bold">交集</div>
                  {overlapHeatmap.map((h, idx) => {
                    ...
                    return (
                       <div 
                        ...
                        style={{
                          left: `${left}%`,
                          width: `${width}%`,
                          opacity: (h.count - 1) / (activeUsersOnDay.length - 1) // Normalize opacity
                        }}
                       ></div>
                    );
                  })}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-white rounded-xl p-1 shadow-sm border border-slate-100">
          <button onClick={() => setCurrentDate(subMonths(currentDate, 1))} className="p-2 hover:bg-slate-50 rounded-lg text-slate-500">
            <ChevronLeft size={20} />
          </button>
          <span className="font-bold text-slate-700 min-w-[140px] text-center">
            {format(currentDate, 'yyyy MMMM', { locale: zhTW })}
          </span>
          <button onClick={() => setCurrentDate(addMonths(currentDate, 1))} className="p-2 hover:bg-slate-50 rounded-lg text-slate-500">
      ...
      <div className="space-y-4">
        {sortedDayData.map(data => (
          <DayRow key={data.dateStr} data={data} />
        ))}
        {!isLoading && sortedDayData.length === 0 && (
          <div className="text-center py-20 bg-white rounded-[2rem] border border-slate-100">
            <p className="text-slate-400 font-bold text-lg">這個月還沒有人填寫時間喔！</p>
