...
import { ChevronLeft, ChevronRight, Clock, Plus, Trash2, X, Loader2, Save, CheckCircle, Edit3, AlertTriangle } from 'lucide-react';
...
export const Schedule: React.FC<ScheduleProps> = ({ currentUser }) => {
  ...
  const isAllDay = useMemo(() => {
    return currentRanges.length === 1 && currentRanges[0].start === 0 && currentRanges[0].end === 24;
  }, [currentRanges]);
  ...
  const handleSaveRanges = async (newRanges: TimeRange[]) => {
    ...
    const existingIndex = newSchedule.findIndex(s => s.date === selectedDateStr);
    ...
    setIsSaving(true);
    try {
      await storageService.saveUserDaySchedule(currentUser.name, selectedDateStr, newRanges);
    } catch (e) {
      alert("儲存失敗，請重試");
    } finally {
      setIsSaving(false);
  ...
  const isOverlapping = (newStart: number, newEnd: number) => {
    return currentRanges.some(r => {
      // Check if (newStart, newEnd) overlaps with (r.start, r.end)
  ...
  const toggleAllDay = (checked: boolean) => {
    ...
    if (checked) {
      ...
      // Revert to default 12-24 suggestion or clear
  ...
  const getDayClass = (day: Date) => {
    ...
    const hasData = schedule.some(s => s.date === dateStr && s.ranges.length > 0);
    const isTodayDate = isToday(day);

    let base = "h-14 md:h-24 w-full rounded-2xl flex flex-col items-center justify-center transition-all relative border-2 btn-bounce ";
    ...
    if (isTodayDate) {
      ...
      base += "bg-white/60 border-transparent text-slate-400 hover:bg-white hover:border-slate-200 ";
    }
    return base;
  };
      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-xl shadow-brand-100/50 border border-white">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2 bg-slate-100 rounded-xl p-1">
            <button onClick={() => setCurrentDate(subMonths(currentDate, 1))} className="p-2 hover:bg-white rounded-lg transition-colors text-slate-600">
              <ChevronLeft size={20} />
            </button>
            <span className="font-bold text-slate-700 min-w-[120px] text-center">
              {format(currentDate, 'yyyy MMMM', { locale: zhTW })}
            </span>
            <button onClick={() => setCurrentDate(addMonths(currentDate, 1))} className="p-2 hover:bg-white rounded-lg transition-colors text-slate-600">
        ...
        <div className="grid grid-cols-7 gap-2 mb-2 text-center">
          {weekDays.map(d => (
            <div key={d} className="text-sm font-bold text-slate-400 py-2">{d}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-2 md:gap-3">
          ...
          {daysInMonth.map(day => {
            ...
            return (
              ...
              >
                <span className="text-lg md:text-2xl font-bold">{format(day, 'd')}</span>
                {hasData && (
                  <div className={`mt-1 flex items-center gap-1`}>
                    <div className="w-1.5 h-1.5 rounded-full bg-green-400" />
                    {/* Tiny text for desktop only */}
                    <span className="hidden md:block text-[10px] text-green-600 font-bold">已填寫</span>
                  </div>
                )}
              </button>
            );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-6 border-b border-slate-100 pb-4">
              <div className="text-slate-400 font-bold mb-1 text-sm">{format(selectedDate, 'yyyy MMMM', { locale: zhTW })}</div>
            ...
            <div className="max-h-[40vh] overflow-y-auto mb-6 custom-scrollbar pr-2 space-y-3">
              {isLoading ? (
                ...
                currentRanges.map((range, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-cream-50 rounded-2xl border border-brand-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {range.end - range.start === 24 && <span className="text-[10px] bg-brand-100 text-brand-600 px-2 py-0.5 rounded-full inline-block mt-1">整天</span>}
                      </div>
                    </div>
                    <button 
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => toggleAllDay(!isAllDay)}>
                <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-colors ${isAllDay ? 'bg-brand-500 border-brand-500' : 'bg-white border-slate-300'}`}>
                  {isAllDay && <CheckCircle size={14} className="text-white" />}
                </div>
                <label className="font-bold text-slate-600 cursor-pointer select-none flex-1">全天 (00:00 - 24:00)</label>
              </div>
                ...
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 space-y-3">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-white p-2 rounded-xl border border-slate-200">
                      ...
                      >
                        {Array.from({ length: 25 }).map((_, i) => (
                          <option key={i} value={i} disabled={i >= endTime}>{i}:00</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="flex-1 bg-white p-2 rounded-xl border border-slate-200">
                      ...
                      >
                        {Array.from({ length: 25 }).map((_, i) => (
                          <option key={i} value={i} disabled={i <= startTime}>{i}:00</option>
                        ))}
                      </select>
                    </div>
                  </div>
              ...
              <div className="flex justify-between items-center h-6 px-1">
                ...
                {!isSaving && <span className="text-xs text-slate-300">變更會自動儲存</span>}
                <button 
                  onClick={() => setIsModalOpen(false)}
